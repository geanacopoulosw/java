...
public class SchedRequest {
	...
	public static class Request implements Comparable<Object> {
		...
		public static ArrayList<Request> processJob(ArrayList<String> request) {
			ArrayList<Request> processing = new ArrayList<>();
			...
			for (String str : request) {
				if (str.contains("cancel")) {
					...
					makeARequest(minutes, startTime, processing, finished);
		...
		// possible
		public static void makeARequest(int finishedTime, int startTime, ArrayList<Request> processing,
									   ArrayList<Request> finished) {

			if (processing.size() == 0) {
			} else {
				Request next = processing.get(0);
				if (next.startMinute <= finishedTime) {
					if (next.startMinute > startTime) {
						if (finished.size() == 0) {
							finished.add(next);
							processing.remove(next);
							makeARequest(finishedTime, startTime, processing, finished);
						}
						else {
